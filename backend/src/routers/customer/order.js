...
router.get("/customer/order/:id", auth, async (req, res) => {
  try {
    const order = await Order.findOne({ orderId: req.params.id }).populate(
      "seller",
      "verified username storeDescription storeName image createdAt views"
    );
    if (order) {
      ...
      if (
        order.customer &&
        order.customer.toString() !== req.customer._id.toString()
      )
        throw new BadRequestError("Order fulfilled by other customer");
      if (!order.seller.verified)
        return res.status(401).send(new StoreNotVerifiedError());
