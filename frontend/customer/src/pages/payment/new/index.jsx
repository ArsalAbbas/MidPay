...
const NewIssue = () => {
  ...
  const handleCopyLink = async () => {
    await navigator.clipboard.readText().then((text) => setOrderId(text));
  };
  const { id } = useParams();
  React.useEffect(() => {
    if (id) {
    ...
    const options = {
      ...
      handler: async function (response) {
        ...
        if (result.status === 201) {
          setSnackbarMessage(
            "Payment Successful. Redirecting you to main screen..."
          );
          setTimeout(function () {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      setValues({
        ...
        username: order.seller.instagramUsername
          ? order.seller.instagramUsername
      ...
      if (e.response) setSnackbarMessage(e.response.data.message);
      else setSnackbarMessage(e.message);
    } finally {
      setFetching(false);
    }
  };
  const renderStat = (title, value, number) => {
    return <ProgressBar value={value} label={title} number={number} />;
  };
  return (
    <Container>
        ...
        InputProps={{
          startAdornment: (
            <InputAdornment position="start">
              <Typography variant={"primary"}>Order Id :</Typography>
            </InputAdornment>
          ),
          endAdornment: (
          ...
          <Paper className={classes.paper} elevation={5}>
            ...
            >
              ...
              >
                {values.image
                  ? values.image
                  : values.storeName.charAt(0).toUpperCase()}
              </Avatar>
            ...
            <Stack direction={"column"} spacing={1}>
              {renderStat(
                "5 Star",
                (values.r5 / values.reviewCount) * 100,
                values.r5
              )}
              {renderStat(
                "4 Star",
                (values.r4 / values.reviewCount) * 100,
                values.r4
              )}
              {renderStat(
                "3 Star",
                (values.r3 / values.reviewCount) * 100,
                values.r3
              )}
              {renderStat(
                "2 Star",
                (values.r2 / values.reviewCount) * 100,
                values.r2
              )}
              {renderStat(
                "1 Star",
                (values.r1 / values.reviewCount) * 100,
                values.r1
              )}
            </Stack>
            <Divider flexItem sx={{ marginTop: 1, marginBottom: 1 }} />
          ...
          <Paper className={classes.paymentCard} elevation={5}>
            ...
            >
              ...
              >
                ...
                <Typography variant={"h3"}>Rs. {values.amount}</Typography>
